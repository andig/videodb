    ...
    foreach ($output as $line) {
        ...
        if (ereg("^ID_", $line)) {
            if (ereg("^ID_VIDEO_FORMAT=(.*)", $line, $regs)) {
                $video_codec = $regs[1];
                if ($video_codec == '0x10000001')
                    $video_codec = 'MPEG1';
                if ($video_codec == '0x10000002')
                    $video_codec = 'MPEG2';
                if ($video_codec == 'MPG4')
                    $video_codec = 'MPEG4';
                if ($video_codec == 'DIV3')
                ...
                if ($audio_codec == 'a52')
                    $audio_codec = 'AC3';
                if ($audio_codec == 'ffvorbis')
                    $audio_codec = 'Vorbis';
                if ($audio_codec == 'pcm')
                    $audio_codec = 'PCM';
                if ($audio_codec == 'ffwmav2')
                    $audio_codec = 'WMA2';
                if ($audio_codec == 'ffwmav1')
                    $audio_codec = 'WMA1'; // just a guess, needs confirmation
            }
        }
    }
    echo "<tr>
