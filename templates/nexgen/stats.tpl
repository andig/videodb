                ...
                <table class="small-12">
                  <tr>
                    ...
                    <td colspan="2" valign="bottom">
                      {foreach key=year item=count from=$stats.count_year}<a href="search.php?q={$year}&amp;fields=year&amp;nowild=1"><img src="images/bar.gif" style="width:7px; height:{math equation='max(round(100/y*x),1)' x=$count y=$stats.max_count}px;" title="{$year}: {$count}" alt="{$year}: {$count}" /></a>{/foreach}
                    </td>
