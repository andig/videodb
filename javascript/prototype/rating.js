 ...
 * Added rating request enhancements
 *
 ...
 * @version 1.0.1
 *
 * $Id: rating.js,v 1.3 2007/12/30 11:27:12 andig2 Exp $
 */
...
Object.extend(Control.Rating.prototype,{
    ...
    setValue: function(value,force_selected,prevent_callbacks){
        ...
        if(!prevent_callbacks){
            this.notify('onChange',this.value); //!!
            if(this.options.updateUrl){
                var params = this.options.parameters || {}; //!!
                params[this.options.updateParameterName] = this.value;
                var req = new Ajax.Request(this.options.updateUrl,{
                    parameters: params,
                    
                    //!!
                    onSuccess: function(transport) {
                        req.creator.notify('afterChange',req.creator.value);
                    }
                });
                req.creator = this; //!!
            }
            //!! added else
            else this.notify('afterChange',this.value);
        }
    },
